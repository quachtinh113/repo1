...
typedef enum{
	/*Map store*/
  basic_address=ADDR_FLASH_PAGE_47,
	address_process=basic_address,
	address_mode=basic_address+1  
}MapStore;

/* Functions ---------------------------------------------------------*/
extern HAL_StatusTypeDef flash_write(uint32_t address, uint8_t data[],uint16_t size);
extern uint8_t flash_read(uint32_t address);
extern void refresh_flash(void);
extern void read_buffer(uint32_t basic_address,uint8_t *buffer,uint8_t length);
extern uint32_t page_to_address(uint8_t page);
extern void update_flash(uint32_t address, positionDef pos, uint8_t length);
static void hot_current_toggle(void);
